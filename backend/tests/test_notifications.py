...
@pytest.fixture
async def affiliated(client):
    """Alice et Bob avec un compte — donc membres de la Taverne. Sans ça, ce
    sont des extérieurs et plus rien n'est annoncé, ni sur le webhook d'une
    ligue ni sur le canal global (cf. dispatch_to_leagues)."""
    return {name: await _auth(client, name) for name in ("Alice", "Bob")}


async def test_game_finished_notification_dispatched(client, affiliated, fake_httpx):
...
async def test_game_finished_notification_includes_elo_delta(client, affiliated, fake_httpx):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/x"})
...
async def test_game_finished_ranks_by_score_desc_not_submission_order(client, affiliated, fake_httpx):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/x"})
    for name in ("Carol", "Dave"):
        await _auth(client, name)
...
async def test_game_finished_cutthroat_ranks_by_score_asc(client, affiliated, fake_httpx):
...
async def test_idempotent_retry_does_not_renotify(client, affiliated, fake_httpx):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/x"})
...
async def test_tie_game_sends_egalite_message(client, affiliated, fake_httpx):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/x"})
...
async def test_casual_game_is_not_announced(client, affiliated, fake_httpx):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/x"})

    # Une amicale reste dans l'historique perso, mais ne sort pas de l'appli.
...
def test_should_announce_finish():
    from app.schemas.game import GamePlayerRead, GameRead
    from app.services.notifications import should_announce_finish

    def game(**kw):
        base = dict(
            id=uuid.uuid4(), date=datetime(2026, 1, 1, tzinfo=timezone.utc), mode="Cricket",
            variant="Normal", duration=120, winner="Alice", is_casual=False,
            players=[GamePlayerRead(name="Alice", score=10, position=1),
                     GamePlayerRead(name="Bob", score=20, position=2)],
        )
        return GameRead(**{**base, **kw})

    assert should_announce_finish(game())
    assert not should_announce_finish(game(is_casual=True))
    assert not should_announce_finish(game(mode="Bob27"))
    assert not should_announce_finish(
        game(players=[GamePlayerRead(name="Alice", score=27, position=1)])
    )
...
async def _league_with_webhook(client, headers, name, url, members=()):
    created = (await client.post("/leagues", json={"name": name}, headers=headers)).json()
    for member_headers in members:
        resp = await client.post(
            "/leagues/join", json={"code": created["invite_code"]}, headers=member_headers
        )
        assert resp.status_code == 200
    resp = await client.patch(
        f"/leagues/{created['id']}/webhook", json={"webhook_url": url}, headers=headers
    )
    assert resp.status_code == 200
    return resp.json()


async def test_league_webhook_routes_and_skips_global(client, affiliated, fake_httpx):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/global"})
    league = await _league_with_webhook(
        client, affiliated["Alice"], "CONNECTED PRODUCTS", "https://chat.example/league",
        members=[affiliated["Bob"]],
    )
    assert league["webhook_url"] == "https://chat.example/league"

    await client.post("/games", json=GAME)  # Alice ET Bob sont membres actifs
...
async def test_game_with_an_outsider_is_not_announced_to_the_league(client, affiliated, fake_httpx):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/global"})
    # Bob a un compte mais n'entre pas dans la ligue : il reste un extérieur.
    await _league_with_webhook(
        client, affiliated["Alice"], "Ligue A", "https://chat.example/league"
    )
...
async def test_ghost_member_counts_as_an_outsider(client, affiliated, fake_httpx):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/global"})
    league = await _league_with_webhook(
        client, affiliated["Alice"], "Ligue A", "https://chat.example/league",
        members=[affiliated["Bob"]],
    ...
    # Bob quitte la ligue (is_active=False) : la table n'est plus complète.
    mine = (await client.get("/leagues/mine", headers=affiliated["Bob"])).json()
    members = next(row for row in mine if row["id"] == league["id"])["members"]
    bob_id = next(m["id"] for m in members if m["name"] == "Bob")
    resp = await client.delete(f"/leagues/{league['id']}/members/{bob_id}", headers=affiliated["Bob"])
    assert resp.status_code == 204
    fake_httpx.calls = []

    await client.post("/games", json={**GAME, "id": "33333333-3333-3333-3333-333333333333"})
    assert fake_httpx.calls == []


async def test_casual_game_never_reaches_a_league_webhook(client, affiliated, fake_httpx):
    await _league_with_webhook(
        client, affiliated["Alice"], "Ligue A", "https://chat.example/league",
        members=[affiliated["Bob"]],
    )

    await client.post("/games", json={**GAME, "is_casual": True})
    assert fake_httpx.calls == []
...
async def test_two_leagues_same_url_announce_once(client, affiliated, fake_httpx):
    for name in ("Ligue A", "Ligue B"):
        await _league_with_webhook(
            client, affiliated["Alice"], name, "https://chat.example/shared",
            members=[affiliated["Bob"]],
        )
...
async def test_game_start_announced_with_watch_link(client, affiliated, fake_httpx, instant_announce):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/x"})
...
async def test_result_is_posted_as_a_reply_in_the_game_thread(client, affiliated, fake_httpx, instant_announce):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/x"})
...
async def test_game_without_live_match_is_not_threaded(client, affiliated, fake_httpx):
    await client.post("/webhooks", json={"target": "google_chat", "url": "https://chat.example/x"})
...
async def test_abandoned_match_closes_its_thread(client, affiliated, fake_httpx, instant_announce):
    from app.services import live
...
async def test_explicit_abort_over_the_socket_closes_the_thread(client, affiliated, fake_httpx, instant_announce):
...
async def test_discord_only_posts_the_result(client, affiliated, fake_httpx, instant_announce):
    await client.post("/webhooks", json={"target": "discord", "url": "https://discord.example/x"})
